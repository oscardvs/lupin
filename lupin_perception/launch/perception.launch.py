"""Launch the `tag_annotator` AprilTag detector.
...
For simulation, point `image_topic`/`camera_info_topic` at the Gazebo camera
plugin's topics and set `fallback_intrinsics` so PnP still runs if CameraInfo
is late. (The full sim stack wires this up in
`lupin_bringup/launch/sim_full.launch.py`.)
"""
    ...
    args = [
        ...
        DeclareLaunchArgument(
            ...
            description='Physical edge length of the printed tags, in metres. '
                        ...
                        'output (and the broadcast tag→camera TF) by a '
                        'constant factor.',
        ),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            ...
            description='QoS profile for the image subscription: '
                        ...
                        'sim-style profile (Gazebo camera plugin).',
        ),
        DeclareLaunchArgument(
            'use_sim_time', default_value='false',
            description='Set true in sim (Gazebo /clock) or when replaying a '
                        'bag with /clock; the real robot has no /clock '
                        'publisher.',
        ),
    ]
